...
export default function RecurringExpensesPage() {
  ...
  const load = async () => {
    ...
    let list = (data ?? []) as RecurringExpense[];
    try {
      list = await patchUnconvertedRates(
        list,
        (r) => r.next_run_date,
        async (id, rate) => { await sb().from('recurring_expenses').update({ exchange_rate: rate }).eq('id', id); },
      );
  ...
  async function save() {
    ...
    let exchange_rate = currency === 'INR' ? 1 : Number(f.exchange_rate ?? 1);
    if (rateLooksUnconverted(currency, exchange_rate)) {
      try { exchange_rate = (await quoteInr(currency, f.next_run_date)).rate; }
  ...
  async function togglePause(r: RecurringExpense) {
    if (r.is_active) {
      if (!(await confirm(`Pause "${r.title}"? The nightly job will skip it until you resume — like pulling a circuit breaker. Expenses already in the books stay.`))) return;
    }
    const { error } = await sb().from('recurring_expenses').update({ is_active: !r.is_active }).eq('id', r.id);
    if (error) return toast(error.message, 'error');
    toast(r.is_active
      ? 'Paused — it will not log or count in run-rate until you resume'
      : 'Resumed — due dates will log again', r.is_active ? 'info' : 'success');
    load();
  }
  ...
  return (
    <>
      <PageHeader title="Subscriptions"
        subtitle="Recurring vendor spend — Cursor, AWS, Adobe. Money out, distinct from Retainers (invoices in). Dollar burn is converted to INR at the closest rate on the bill date. Pause if the plan might stop.">
        ...
        {loading ? <Loading />
          : rows.length === 0 ? (
            <EmptyState icon={<CreditCard size={18} />} title="No subscriptions yet"
              body="Log Cursor, Anthropic, Adobe, AWS once — a matching expense appears on schedule and counts toward burn and runway in INR."
            ...
            <div className="scroll-x">
              <table className="w-full min-w-[920px]">
                <thead><tr className="bg-ink-800/60">
                  ...
                  <th className="th text-right">/ month INR</th><th className="th w-52"></th>
                </tr></thead>
                <tbody>
                  {rows.map((r) => {
                    const monthly = Number(r.taxable_amount) * (FREQ_PER_MONTH[r.frequency] ?? 1) * (Number(r.exchange_rate) || 1);
                    return (
                      <tr key={r.id} className={r.is_active ? 'row-link' : 'row-link opacity-[0.48]'}>
                        <td className="td">
                        ...
                        <td className="td text-right font-mono tabular-nums text-[13px] text-white">
                          {money(r.taxable_amount, r.currency)}
                        </td>
                        <td className="td text-right font-mono tabular-nums text-[13px] text-amber-300">
                          <span className="block">{moneyShort(monthly)}</span>
                          {r.currency !== 'INR' && (
                            <span className="block text-[10.5px] font-normal text-chrome-dark">
                              {r.currency} × {Number(r.exchange_rate).toFixed(2)}
                            </span>
                          )}
                        </td>
                        <td className="td">
                          <div className="flex justify-end gap-1">
                            <button className="btn-subtle btn-xs" title={r.is_active ? 'Log expense now' : 'Resume first to log'}
                              onClick={() => generate(r.id)} disabled={!r.is_active || busy === r.id}>
                              {busy === r.id ? <Spinner size={13} /> : <Zap size={14} />}
                            </button>
                            <button className="btn-subtle btn-xs" title={r.is_active ? 'Pause this subscription' : 'Resume'}
                              onClick={() => togglePause(r)}>
                              {r.is_active ? <><PauseCircle size={14} /> Pause</> : <><PlayCircle size={14} /> Resume</>}
                            </button>
                            <button className="btn-subtle btn-xs" title="Edit" onClick={() => { setF({ ...r }); setOpen(true); }}><Pencil size={14} /></button>
                            <button className="btn-subtle btn-xs text-red-400" title="Delete" onClick={() => remove(r)}><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <Modal open={open} onClose={() => setOpen(false)} width="max-w-2xl"
        ...
        subtitle="Same GST split as a one-off expense. The nightly cron logs it when next-run is due. Pause if the vendor plan might stop."
        footer={<>
          ...
          <div className="sm:col-span-2 flex flex-wrap items-center justify-between gap-4 rounded-lg border border-line bg-ink-800/50 px-4 py-3">
            <div className="flex flex-wrap gap-6">
              <Toggle checked={!!f.is_active} onChange={(v) => setF({ ...f, is_active: v })}
                label={f.is_active ? 'Active' : 'Paused'} />
            ...
            <p className="font-mono text-[13px] text-white">
              ...
              {f.currency !== 'INR' && Number(f.exchange_rate) > 1 && (
                <span className="ml-2 text-amber-300">≈ {money(tax.total_amount * Number(f.exchange_rate))}</span>
              )}
